...
from .parameters import focal_length, max_sun_angle, n_pix, n_sca, pixel_size_mm

# Basic Roman reference info, with lengths in mm.
pix_scale = (pixel_size_mm / focal_length) * coord.radians
n_sip = (
    5  # Number of SIP coefficients used, where arrays are n_sip x n_sip in dimension
)
...
def _parse_sip_file(file):
    ...
    if not os.path.exists(file):
        raise OSError(
            "Cannot find file that should have Roman SIP coefficients: %s" % file
        )
...
def findSCA(wcs_dict, world_pos, include_border=False):
    ...
    bounds_list = [
        BoundsI(x1, x2, y1, y2) for x1, x2, y1, y2 in zip(xmin, xmax, ymin, ymax)
    ]
