...
from romanisim import catalog, image, log, persistence, util  # , parameters, psf, wcs
from romanisim.models import parameters, psf, wcs
...
def test_trim_objlist():
    ...
    affine = galsim.AffineTransform(
        ...
        0,
        0.1,
        origin=galsim.PositionI(50, 50),
        world_origin=galsim.PositionD(0, 0),
    )
    wcs = galsim.TanWCS(affine, util.celestialcoord(cen))
...
def test_make_l2():
    ...
    gain = 1  # electron/DN
    slopes, readvar, poissonvar = image.make_l2(
        resultants, read_pattern, gain=gain, flat=1, darkrate=0
    )
...
def central_stamp(im, sz):
    ...
    szo2 = sz // 2
    odd = sz % 2
    return im[center - szo2 : center + szo2 + odd, center - szo2 : center + szo2 + odd]
...
def test_image_rendering():
    ...
    # by one pixel.
    off = (oversample - 1) / 2
    xx, yy = np.meshgrid(np.arange(imsz * oversample), np.arange(imsz * oversample))
    mod = Sersic2D(
        amplitude=1,
        r_eff=oversample * sersic_hlr / wfi.pixelscale,
        n=sersic_index,
        x_0=pos[0] * oversample + off,
        y_0=pos[0] * oversample + off,
        ellip=0,
        theta=0,
    )
    modim = mod(xx, yy)
    ...
    integral = (
        1
        * (oversample * sersic_hlr / wfi.pixelscale) ** 2
        * g2n
        * 2
        * np.pi
        * sersic_index
    ...
    log.info(
        "DMS215: rendered galaxy matches astropy Sersic2D after pixel integration."
    )
...
def test_fast_epsf():
    ...
    cat = [
        catalog.CatalogObject(None, galsim.DeltaFunction(), deepcopy({filtname: 1000}))
    ] * len(x)

    impsf = psf.make_psf(
        sca, filtname, psftype="galsim", chromatic=False, variable=True
    )
...
def test_add_objects():
    ...
    assert np.abs(np.sum(im.array) - 2 * counts) < np.hypot(
        5 * np.sqrt(counts), 0.03 * counts
    )
    ...
    image.add_objects_to_image(
        ...
        [30, 30],
        impsfchromatic,
        flux_to_counts_factor=2,
        bandpass=bandpass,
    )
    assert np.abs(np.sum(im.array) - 2 * counts * 2) < np.hypot(
        5 * np.sqrt(counts), 0.03 * 2 * counts
    )
...
def test_simulate_counts_generic():
    ...
    assert np.abs(np.mean(im2.array) - poisson_rate) < 10 * np.sqrt(poisson_rate / npix)
    ...
    assert np.abs(poisson_rate - np.var(im2.array)) < 10 * np.sqrt(var_of_var)
    ...
    assert np.abs(np.mean(im3.array) - exptime) < 20 * np.sqrt(
        skycountspersecond * exptime / npix
    )
    im4 = im.copy()
    image.simulate_counts_generic(im4, exptime, dark=sky, qe=0.5, zpflux=zpflux)
    # verify that dark electrons are not hit by the QE
    assert np.all(im3.array - im4.array < 20 * np.sqrt(exptime * skycountspersecond))
    im5 = im.copy()
    image.simulate_counts_generic(im5, exptime, sky=sky, qe=0.5, zpflux=zpflux)
    # verify that sky photons are hit by the QE
    poisson_rate = skycountspersecond * exptime * 0.5
    assert np.abs(np.mean(im5.array) - poisson_rate) < 20 * np.sqrt(poisson_rate / npix)
...
def test_simulate_counts():
    ...
    im1 = image.simulate_counts(
        meta, chromcat, usecrds=False, psftype="galsim", ignore_distant_sources=4000
    )
    im2 = image.simulate_counts(
        meta, graycat, usecrds=False, psftype="epsf", ignore_distant_sources=4000
    )
    im1 = im1[0].array
    im2 = im2[0].array
...
def test_simulate():
    ...
    filter_name = "F158"
    meta = util.default_image_meta(time=time, filter_name=filter_name, coord=coord)
    ...
    l0 = image.simulate(meta, graycat, psftype="epsf", level=0, usecrds=False)
    ...
    l1 = image.simulate(
        meta, graycat, psftype="epsf", level=1, crparam=dict(), usecrds=False, rng=rng
    )
    ...
    log.info(
        "DMS218: successfully used WCS / focal plane geometry to render "
    ...
    if artifactdir is not None:
        af = asdf.AsdfFile()
        af.tree = {"image": l0[0]["data"], "imloc": peakloc, "trueloc": sourcecen}
        af.write_to(os.path.join(artifactdir, "dms218.asdf"))

    rng = galsim.BaseDeviate(1)
    l1_nocr = image.simulate(
        meta, graycat, psftype="epsf", level=1, usecrds=False, crparam=None, rng=rng
    )
    assert np.all(l1[0].data >= l1_nocr[0].data)
    log.info("DMS221: Successfully added cosmic rays to an L1 image.")
    l2 = image.simulate(
        meta, graycat, psftype="epsf", level=2, usecrds=False, crparam=dict()
    )
    # throw in some CRs for fun
    l2c = image.simulate(meta, chromcat, psftype="galsim", level=2, usecrds=False)
    ...
    roughguess = persistence.fermi(
        fluence,
        170,
        parameters.persistence["A"],
        parameters.persistence["x0"],
        parameters.persistence["dx"],
        parameters.persistence["alpha"],
        parameters.persistence["gamma"],
    )
    roughguess = roughguess * 140  # seconds of integration
    gain = parameters.reference_data["gain"]
    assert np.abs(np.log(np.mean(diff * gain) / roughguess)) < 1
...
def test_make_test_catalog_and_images():
    ...
    if fn is not None:
        fn = str(fn)
    res = image.make_test_catalog_and_images(
        usecrds=False,
...
def test_reference_file_crds_match(level):
    ...
    rd_sca = twcs.toWorld(
        galsim.PositionD(galsim.roman.n_pix / 2, galsim.roman.n_pix / 2)
    )
    ...
    im, simcatobj = image.simulate(
        metadata, cat, usecrds=True, psftype="epsf", level=level, rng=rng
    )
...
def test_inject_source_into_image():
    ...
    meta = util.default_image_meta(
        coord=coord, filter_name=filt, detector="WFI07", ma_table=4
    )
    wcs.fill_in_parameters(meta, coord)
    rng_seed = 42
    rng = galsim.UniformDeviate(rng_seed)
    cat = catalog.make_dummy_table_catalog(
        coord, radius=0.1, bandpasses=[filt], nobj=2000, rng=rng
    )
    # Create starting image
    im, simcatobj = image.simulate(
        meta, cat, usecrds=False, psftype="epsf", level=2, rng=rng, crparam=None
    )
    ...
    assert np.all(
        (
            im.data[ypos - 1 : ypos + 2, xpos - 1 : xpos + 2]
    ...
    fluxeps = flux * romanisim.bandpass.get_abflux(
        "F158", int(meta["instrument"]["detector"][3:])
    )  # electron/s
    assert (
        np.abs(
            np.sum(iminj.data - im.data) * parameters.reference_data["gain"] / fluxeps
            - 1
        )
        < 0.1
    )
    ...
    log.info(
        f"DMS231: successfully injected a source into an image at x,y = {xpos},{ypos}."
    )

    artifactdir = os.environ.get("TEST_ARTIFACT_DIR", None)
    if artifactdir is not None:
        af = asdf.AsdfFile()
        af.tree = {"originalimage": im, "newimage": iminj, "catinj": catinj, "cat": cat}
...
def test_image_input(tmpdir):
    ...
    offsets = np.array([[-300, 300, -100, -200, 0, 0], [0, 100, -200, -100, 0, -300]])
    ...
    expectedflux = (
        romanisim.bandpass.get_abflux("F087", int(meta["instrument"]["detector"][3:]))
        * np.sum(tab["F087"])
    ...
    log.info(
        "DMS228: Successfully added rendered sources from image input; "
...
def test_psftypes_similar(psftype):
    ...
    s = 10  # box half-size
    galsim_sum = np.sum(
        galsim_image[center[0] - s : center[0] + s, center[1] - s : center[1] + s]
    )
    image_sum = np.sum(
        image[center[0] - s : center[0] + s, center[1] - s : center[1] + s]
    )
    flux_ratio = image_sum / galsim_sum
    log.info(
        f"test_psftypes_similar: {psftype} vs galsim flux ratio = {flux_ratio:.6f}, "
        f"difference = {abs(flux_ratio - 1.0):.6f}"
    )
    assert (abs(flux_ratio - 1.0)) < 0.1  # This isn't great but sufficient.
...
def set_up_image_rendering_things(psftype="epsf"):
    ...
    impsfgray = psf.make_psf(
        1, filter_name, psftype=psftype, chromatic=False, **psf_keywords
    )
    impsfchromatic = psf.make_psf(1, filter_name, psftype="galsim", chromatic=True)
    ...
    graycatalog = [
        ...
        catalog.CatalogObject(
            None, galsim.Sersic(1, half_light_radius=0.2), deepcopy(fluxdict)
        ),
    ]
