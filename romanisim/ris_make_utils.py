...
from romanisim import catalog, image, log  # , wcs, parameters
...
def set_metadata(
    ...
    if usecrds:
        try:
            ...
            context = None
        ref = crds.getreferences(
            {
                "ROMAN.META.INSTRUMENT.NAME": "wfi",
                "ROMAN.META.EXPOSURE.START_TIME": meta["exposure"][
                    "start_time"
                ].value,
            },
            reftypes=["matable"],
        ...
        meta["exposure"]["read_pattern"] = matab["roman"]["science_tables"][
            f"SCI{ma_table_number:04}"
        ]["science_read_pattern"]
    else:
        meta["exposure"]["read_pattern"] = parameters.read_pattern[
            ma_table_number
        ]

    if truncate is not None:
        meta["exposure"]["read_pattern"] = meta["exposure"]["read_pattern"][
            :truncate
        ]
    ...
    position, velocity = coordinates.get_body_barycentric_posvel(
        "earth", meta["exposure"]["start_time"]
    )
...
def create_catalog(
    ...
    if distortion_file is not None:
        dist_model = roman_datamodels.datamodels.DistortionRefModel(
            distortion_file
        )
    ...
    if catalog_name is None:
        ...
        if metadata:
            ...
            if "exposure" in metadata:
                ...
                # Level 3
                date = time.Time(
                    metadata["coadd_info"]["time_mean"], format="mjd"
                )
        else:
            date = None

        cat = catalog.read_catalog(
            catalog_name,
            coord,
            date=date,
            radius=radius,
...
def parse_filename(filename):
    ...
    if match is None:
        return None
    out = dict(
        observation_id=filename.replace("_", "")[1:],
...
def simulate_image_file(
    args, metadata, cat, rng=None, persist=None, psf_keywords=dict(), **kwargs
):
    """
    ...
    if getattr(args, "webbpsf", False) or getattr(args, "stpsf", False):
        log.warning(
            "Warning: webbpsf and stpsf arguments are deprecated, please use "
...
def parse_apt_file(filename):
    ...
    keys = [
        (("ProgramInformation", "Title"), ("program", "title")),
        (
            (
                "ProgramInformation",
        ...
        (("ProgramInformation", "ProgramCategory"), ("program", "category")),
        (
